    ...
    yup.object().shape({
      ...
      lessons: yup.array().of(
        yup.object().shape({
          ...
          _id: yup.string(), // Add _id field for existing lessons
          subLessons: yup.array().of(
            yup.object().shape({
              ...
              file: yup.mixed(), // Remove required validation here
              existingFile: yup.string(),
              _id: yup.string(), // Add _id field for existing sublessons
...
const CourseForm = () => {
              ...
              subLessons: [
                {
                  ...
                  existingFile: "",
                  _id: "", // Initialize with empty string
                  additionalFiles: [{ file: null, name: "", existingFile: "" }],
                  links: [{ url: "", name: "" }],
                },
              ],
            },
  ...
  const mapDataToForm = (courseData) => {
    ...
    // Prepare languages array for form
      ...
      return {
        ...
        lessons: langLessons.map(lesson => {
          const formattedSubLessons = lesson.subLessons.map(subLesson => {
            ...
            const additionalFiles = subLesson.additionalFiles?.length > 0 
              ? subLesson.additionalFiles.map(af => ({
                  file: null,
                  name: af.name || "",
                  existingFile: af.file || ""
                }))
              : [{ file: null, name: "", existingFile: "" }];

            // Format links
            const links = subLesson.links?.length > 0
            ...
            return {
              ...
              name: subLesson.name || "",
              description: subLesson.description || "", // Ensure description is mapped correctly
              file: null,
              existingFile: subLesson.file || "",
              _id: subLesson._id || "", // Store the subLesson _id
              additionalFiles,
              links
            };
          });
          ...
          return {
            ...
            name: lesson.name,
            _id: lesson._id || "", // Store the lesson _id
            subLessons: formattedSubLessons.length > 0 ? formattedSubLessons : [
              {
                srNo: `${lesson.srNo}.1`,
                name: "",
                description: "",
      ...
      try {
        ...
        for (const language of data.languages) {
          ...
          const languageLessons = language.lessons.map((lesson, lessonIndex) => {
            // Create base lesson object with _id if available
            const lessonObj = {
              name: lesson.name,
            ...
            // Add _id if it exists (for existing lessons)
            if (lesson._id) {
            ...
            lessonObj.subLessons = lesson?.subLessons?.map((subLesson, subIndex) => {
              ...
              const transformedSubLesson = {
                ...
                srNo: subIndex + 1,
                file: subLesson.existingFile || null, // Use existing file if available
                additionalFiles: subLesson.additionalFiles
                  .filter(af => af.file || af.name || af.existingFile) // Filter out empty entries
                  .map(af => ({
                    file: af.existingFile || null, // Use existing file if available
                    name: af.name
                  })),
                links: subLesson.links
                  ...
                  _id: null // Add this line
              };
              
              // Add _id if it exists (for existing sublessons)
              if (subLesson._id) {
                transformedSubLesson._id = subLesson._id;
        ...
        for (const lesson of transformedLessons) {
          ...
          for (const subLesson of lesson.subLessons) {
            ...
            // Process main file - only upload if there's a new file
            ...
            // Process additional files - only upload new files
        ...
        console.log('Transformed payload:', transformedLessons);
      ...
      <div className="space-y-4">
              ...
              >
                ...
                {["eng", "kaz", "rus"].map((lang) => (
                  <option 
                    key={lang} 
                    value={lang} 
                    disabled={selectedLanguages.includes(lang) && watch(`languages.${languageIndex}.language`) !== lang}
                  >
                    {lang.toUpperCase()}
                  </option>
                ))}
        ...
        <div className="flex justify-end gap-4 mt-6">
          <button 
            ...
            onClick={() => { window.location.href = "/admin/book-hub" }}
          >
            Cancel
          </button>
          <button 
          ...
          <div className="flex w-full gap-[20px]">
            <div className="flex flex-col gap-[5px]">
              ...
              <input
                ...
                disabled
              />
              {/* Hidden field for lesson _id */}
              <input
                type="hidden"
                {...register(`languages.${nestIndex}.lessons.${index}._id`)}
              />
            </div>
            ...
            <div className="flex items-end">
              <button 
                type="button" 
                onClick={() => {remove(index); handleDeleteLesson(`languages.${nestIndex}.lessons.${index}._id`)}} 
          ...
          <div key={subField.id} className="rounded-[10px] border-2 border-[#EEE] p-4 space-y-3 w-full mb-5">
            <div className="h-11 flex gap-[10px] w-full">
              <input
                ...
                disabled
              />
              {/* Hidden field for subLesson _id */}
              <input
                type="hidden"
                {...register(`languages.${nestIndex}.lessons.${lessonIndex}.subLessons.${subIndex}._id`)}
              />
              <div className="w-[50%]">
                <input
                ...
                <div className="w-full">
                  {existingFile && (
                    <div className="mb-2 text-xs text-gray-500">
                      Existing file: {existingFile.split('/').pop()}
                      <input
                        type="hidden"
                        {...register(`languages.${nestIndex}.lessons.${lessonIndex}.subLessons.${subIndex}.existingFile`)}
                      />
                    </div>
                  )}
                  <CustomFileUpload 
                      ...
                      <div key={fileIndex} className={`flex gap-[10px] ${fileIndex !== 0 ? "pr-[55px]" : ""}`}>
                        <div className="w-[50%]">
                          {existingAdditionalFile && (
                            <div className="mb-2 text-xs text-gray-500">
                              Existing file: {existingAdditionalFile.split('/').pop()}
                              <input
                                type="hidden"
                                {...register(`languages.${nestIndex}.lessons.${lessonIndex}.subLessons.${subIndex}.additionalFiles.${fileIndex}.existingFile`)}
                              />
                            </div>
                          )}
                          <CustomFileUpload
    ...
    <div className={`mb-4 w-[${width}]`}>
      <div className="relative border rounded-lg p-3 flex items-center bg-white cursor-pointer">
        ...
        <span className="flex-1 text-[#6e6e6e] opacity-0.5 text-sm font-normal">
          {fileName ? fileName : aditionalFile === true ? "Additional files" : "Select File"}
        </span>
...
// import { generateSignedUrlCoursesFiles, generateSignedUrlCourseAdditionalFiles } from "@/actions";
// import { toast } from 'sonner';
// import { addNewCourse, getSingleCourse } from "@/services/admin-services";
// import useSWR from "swr";
// import { useParams } from "next/navigation";
...
//         console.log('Transformed payload:', finalPayload);
