...
const BooksEvents = () => {
  ...
  const { data, error, isLoading, mutate } = useSWR(searchQuery?`/admin/events?description=${searchQuery}&${query}`:`/admin/events?${query}`, getBookEvents);
  ...
  return (
    <div>
      ...
      <div className="max-w-7xl mx-auto mb-8 flex items-center justify-end gap-4">
        ...
        <button className="flex items-center gap-2 bg-[#F96915] text-white px-4 py-2 rounded-full hover:bg-[#F96915] transition-colors" onClick={() => router.push("/admin/book-events/add")}>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {events?.map((event, index) => (
            <div key={index} className="  overflow-hidden  " onClick={() => router.push(`/admin/book-events/${event?._id}`)}>
              <div className="aspect-square relative ">
                {/* <Image
                  src={getProfileImageUrl(event.image)}
                  alt={event.name}
                  layout="fill"
                  objectFit="cover"
