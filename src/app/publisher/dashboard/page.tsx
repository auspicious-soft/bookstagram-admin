...
const Page = () => {
  ...
  // Fetch all data for the year without pagination
  const { data, error, isLoading } = useSWR(`/publisher/dashboard?year=${selectedYear}`, getPublisherDashboard);
  ...
  // Paginate from frontend
  const paginatedBooks = allBooks.slice((page - 1) * itemsPerPage, page * itemsPerPage);
              ...
              <div key={book?._id} className='flex gap-[15px] items-center'>
                <div>
                  <Image
                    ...
                    width={62}
                    height={62}
                    className='rounded-[5px] aspect-square'
                  />
                </div>
          ...
          <table>
            ...
            <tbody>
              {isLoading ? (
                ...
                paginatedBooks.map((row: any) => (
                  <tr key={row?._id}>
                    <td><TableRowImage image={row?.image ? getProfileImageUrl(row?.image) : profile} /></td>
                    <td>{row?.name?.eng}</td>
                    <td>{row?.authorId[0]?.name?.eng ?? row?.authorId[0]?.name?.kaz ?? row?.authorId[0]?.name?.rus}</td>
                    <td>{row?.averageRating}</td>
                    <td>
                      <div className="flex flex-wrap gap-2">
                        {(row?.genre || []).slice(0, 3).map((categoryName: string, index: number) => (
